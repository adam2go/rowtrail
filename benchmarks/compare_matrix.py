...
p=argparse.ArgumentParser();p.add_argument('--variant',action='append',required=True);p.add_argument('--rows',type=int,default=1048576);p.add_argument('--repeats',type=int,default=7);p.add_argument('--partitions',type=int,choices=range(1,9));p.add_argument('--variant-partitions',action='append',default=[],help='name=target overrides one variant');p.add_argument('--output',required=True);a=p.parse_args()
...
targets={name:int(value) for name,value in (v.split('=',1) for v in a.variant_partitions)}
assert all(name in variants and target in range(1,9) for name,target in targets.items())
with tempfile.TemporaryDirectory(prefix='rowtrail-matrix-') as td:
 for repeat in range(a.repeats):
  ...
  for name in names:
   ...
   target=targets.get(name,a.partitions)
   subprocess.run([sys.executable,str(ROOT/'benchmarks/compare.py'),'--entry','session','--rows',str(a.rows),'--repeats','1','--bin-dir',variants[name],'--output',str(path),*(['--partitions',str(target)] if target else [])],check=True,stdout=subprocess.DEVNULL)
