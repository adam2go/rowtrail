...
import duckdb
p=argparse.ArgumentParser();p.add_argument('--rows',type=int,default=1048576);p.add_argument('--files',type=int,default=16);p.add_argument('--repeats',type=int,default=5);p.add_argument('--output',default='benchmarks/local/progressive.json');args=p.parse_args()
root=pathlib.Path(__file__).resolve().parents[1];bins=root/'target/release';records=[]
...
with tempfile.TemporaryDirectory(prefix='rowtrail-progressive-bench-') as td:
 ...
 for i in range(args.files):
  con.execute('COPY (SELECT id,amount FROM read_parquet($source) WHERE id >= $low AND id < $high) TO $destination (FORMAT PARQUET, COMPRESSION SNAPPY)',{'source':str(base/'data/many.parquet'),'destination':str(fragments/f'{i:03}.parquet'),'low':9007199254740993+i*width,'high':9007199254740993+(i+1)*width})
 con.close()
 for repeat in range(args.repeats):
  for mode in (['sql','progressive'] if repeat%2==0 else ['progressive','sql']):
   ...
   if mode=='progressive':result=call('analyze',{'source':binding,'aggregates':aggregates,'execution':{'wait_ms':0}})
   ...
   if mode=='progressive':
    progress=first_snapshot['quality']['coverage']['input_coverage'];n=min(args.rows,progress['completed_files']*width);cents=sum(i%2001-1000 for i in range(n) if i%17);non_null=sum(i%17!=0 for i in range(n));mean=format((Decimal(cents)/100/non_null).quantize(Decimal('.000001'),rounding=ROUND_DOWN),'.6f')
