...
| 改动 | 实测结果 |
|---|---:|
| 完整响应 → 精简响应 | **4,706 → 3,461 个响应 token，减少 26.5%** |
| 请求与响应中位数之和 | **减少 20.9%** |
| 返回相关字段，而非完整 64 列 schema | **1,067 → 138 token** |
| 200 万行保存子集 + 十次追问 + 接续 | **683.78 → 689.54 ms**（beta.2 → beta.3） |
| 100 万行五查询探索 | **144.62 → 146.23 ms** |

这里使用 `o200k_base` 对实际 JSON 消息分词，未计模型提示、推理、工具定义、缓存及聊天包装，
不等同于真实模型账单。两种 tokenizer 的结果和所有原始消息均公开；已有大任务整体稳定，
约 0.8% / 1.1% 的小幅退化也保留在报告中。

![beta.3 的上下文节省和更快的直接 SQL 对照](benchmarks/performance/beta3/performance.svg)

直接 DuckDB 仍然更快、返回信息更少：相同任务中，保留内存连接为 **18.12 ms**，
持久数据库关闭后重连为 **34.60 ms**，RowTrail 精简模式为 **72.14 ms**。对照均可保留
