...
同一台 Mac，10 万行任务，七轮实测：

| 改动 | 实测结果 |
|---|---:|
| 完整响应 → 精简响应 | **4,753 → 3,438 个响应 token，减少 27.7%** |
| 请求与响应中位数之和 | **减少 22.2%** |
| 返回相关字段，而非完整 64 列 schema | **1,060 → 127 token** |
| 200 万行保存子集 + 十次追问 + 接续 | **698.81 → 700.08 ms**（beta.2 → beta.3） |
| 100 万行五查询探索 | **146.32 → 144.91 ms** |

这里使用 `o200k_base` 对实际 JSON 消息分词，未计模型提示、推理、工具定义、缓存及聊天包装，
不等同于真实模型账单。两种 tokenizer 的结果和所有原始消息均公开；已有大任务整体稳定，
本轮追问耗时增加约 0.2%，探索减少约 1.0%；前几轮记录及退化也完整保留。

![beta.3 的上下文节省和更快的直接 SQL 对照](benchmarks/performance/beta3/performance.svg)

直接 DuckDB 仍然更快、返回信息更少：相同任务中，保留内存连接为 **17.44 ms**，
持久数据库关闭后重连为 **34.38 ms**，RowTrail 精简模式为 **71.88 ms**。对照均可保留
中间表、先聚合再返回小答案。RowTrail 提供统一的质量、固定版本、用途与交接合约，
这些能力有真实的时间和上下文成本。

SQLite FULL、fsync、SHA-256、数值边界检查和真实取消均保留。最近的真实 Agent 配对实验
仍是 alpha.6：12 次答案均正确，但时间与累计输入 token 高于持久 DuckDB。
本轮脚本分词测量不能替代真实 Agent 实验。
[完整验证及限制](docs/verification.md) · [原始记录与复现](benchmarks/performance/beta3/)。
