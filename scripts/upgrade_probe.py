"""Verify the one-way schema-3 -> schema-4 path using real alpha.3 and alpha.4 binaries."""
import argparse,json,os,pathlib,signal,sqlite3,subprocess,tempfile,time
p=argparse.ArgumentParser();p.add_argument('old_bin_dir');p.add_argument('new_bin_dir');p.add_argument('--report',default='benchmarks/local/upgrade.json');a=p.parse_args()
...
with tempfile.TemporaryDirectory(prefix='rowtrail-upgrade-') as td:
 ...
 try:
  ...
  with sqlite3.connect(ws/'metadata.sqlite') as c:assert c.execute("SELECT value FROM meta WHERE key='schema_version'").fetchone()[0]=='3'
  ...
  with sqlite3.connect(ws/'metadata.sqlite') as c:assert c.execute("SELECT value FROM meta WHERE key='schema_version'").fetchone()[0]=='4'
  ...
  report={'status':'passed','old_version':subprocess.check_output([str(old/'rowtrail'),'--version'],text=True).strip(),'new_version':subprocess.check_output([str(new/'rowtrail'),'--version'],text=True).strip(),'old_fixed_revision_preserved':True,'new_query_on_old_result':True,'old_runtime_rejects_upgraded_store':True}
